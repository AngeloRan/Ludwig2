...
def render_vite_assets():
    ...
    manifest_json_path = os.path.join(
        settings.VITE_STATIC_BUNDLE, ".vite", "manifest.json")
    try:
        with open(manifest_json_path, "r") as f:
            manifest = json.load(f)
            payload = f"""<script type="module" src="/static/{
                settings.VITE_BUILD_DIRNAME}/{manifest['index.html']['file']}"></script>"""
            if "css" in manifest["index.html"].keys():
                payload += f"""<link rel="stylesheet" type="text/css" href="/static/{
                    settings.VITE_BUILD_DIRNAME}/{manifest['index.html']['css'][0]}" />"""
